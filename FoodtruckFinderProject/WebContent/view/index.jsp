<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FoodtruckMain</title>
    <!-- Core CSS - Include with every page -->
    <link href="../resources/plugins/bootstrap/bootstrap.css" rel="stylesheet" />
    <link href="../resources/font-awesome/css/font-awesome.css" rel="stylesheet" />
    <link href="../resources/plugins/pace/pace-theme-big-counter.css" rel="stylesheet" />
    <link href="../resources/css/style.css" rel="stylesheet" />
      <link href="../resources/css/main-style.css" rel="stylesheet" />

</head>
    
    <style>
        #indexMain{ margin:60px;}
        #indexbody{margin-left: 350px;}
        #Recommend{margin-left: 700px;}
        #Reviews{margin-left: 700px;}
        #Location{margin-left: 700px;}
        
        .Recommend1{
            display:inline-block;
        }
    </style>

<body>
    
    <!--  wrapper -->
    <div id="wrapper">
        <!-- navbar top -->
        
        <nav class="navbar navbar-default navbar-fixed-top" role="navigation" id="navbar">
            <!-- navbar-header -->
            <br>
            <div class="col-xs-12" style="margin-right: 5px" >
                <span style="float:right"><input type="button" value="Sign up"> </span>
                <span style="float:right"><input type="button" value="Login"> </span>
            </div>
            <ul class="nav navbar-top-links navbar-right">	
                <br>
                <br>
            <div class="col-xs-10" style="margin-right: 550px" >
            <a class="navbar-brand" style="float:center" href="index.html">
                    <img src="../resources/img/logo.png" alt="" />
                </a>
            </div>
            </ul>
            <!-- navbar-top-links -->
            <ul class="nav navbar-top-links navbar-right">
                <li class="row">
                		<div class="col-xs-10" style="margin-right: 350px">
	            			<!-- search section-->
	                        <div class="input-group custom-search-form">
	                        	<input type="text" class="form-control" placeholder="Search" style="width: 400px">
	                            <input type="text" class="form-control" placeholder="Location" style="width: 400px">
	                                <button class="btn btn-default" type="button">
	                                    <i class="fa fa-search"></i>
	                                </button>
	                        </div>
	                		<!--end search section-->
	            		</div>
                </li>
                <br>
                <br>

                <div id="container" align="center" style="margin-right: 300px">
                <img src="../resources/img/user.jpg" />
                <img src="../resources/img/user.jpg" />
                <img src="../resources/img/user.jpg" />
                <img src="../resources/img/user.jpg" />
                </div>

 
            </ul>
            
        </nav>
       <!-- end navbar top -->

    <!-- Core Scripts - Include with every page -->

       
        <!-- end page-wrapper -->

    </div>
    <!-- end wrapper -->

    <!-- Core Scripts - Include with every page -->

    <script src="assets/plugins/jquery-1.10.2.js"></script>
    <script src="assets/plugins/bootstrap/bootstrap.min.js"></script>
    <script src="assets/plugins/metisMenu/jquery.metisMenu.js"></script>
    <script src="assets/plugins/pace/pace.js"></script>
    <script src="assets/scripts/siminta.js"></script>

</body>
      <br>
        <br><br><br><br><br><br><br><br><br><br><br><br>
    <div id="Recommend">
        <u><font size="5">Recommend</font></u>
    </div>
    <br><br>
       <div class="container">
                        <div style="float: left; width: 33%;" >
     <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text>  <br>  
    </div>
    <div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
<div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
           
    </div>
    <br>
    <br>
        <div id="Recommend">
            <u><font size="5">Most Reviews</font></u>
            <br><br>
    </div>
 <div class="container">
                        <div style="float: left; width: 33%;" >
     <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text>  <br>  
    </div>
    <div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
<div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
           
    </div>
    <br>
    <br>
        <div id="Recommend">
            <u><font size="5">Near MyLocation</font></u>
    </div>
    <br>
    <br>
     <div class="container">
                        <div style="float: left; width: 33%;" >
     <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text>  <br>  
    </div>
    <div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
<div style="float: left; width: 33%" >
        <img src="../resources/img/twiceprofile.jpg" /><br>
            <text >푸드트럭</text><br>
         <text>★★★★</text><br>
         <text>foodtruck Cotegory</text> 
        
</div>
           
    </div>
</html>
